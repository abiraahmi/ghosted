...
The ghosted package provides four high-level helpers to de‑identify transcripts without forcing you to work with intermediate data.frames:
...
- `redact_interviewer`: if `TRUE`, also redact interviewer names in text.
- `include_common_names`: if `TRUE`, include `ghosted::common_names_default()` if available.
...
- `output_path`: explicit output file path; if `NULL`, uses input dir with `suffix`.
...
- `report_redacted`: if `TRUE`, prints which phrases were found and redacted.
...
Speaker labels at the start of a line/paragraph are normalized to `Interviewer` or `Participant` (e.g., `Name:` or `<v Name>` → `Interviewer:`), while names elsewhere are replaced with `[REDACTED]`.
...
- `ghost_batch()`: Progress is shown with a console progress bar.
- For DOCX/TXT → VTT conversions (when `out_format = "vtt"`), cues are written without timestamps.
- Leading speaker tokens at line start (e.g., `Name:` or
`<v Name>`) are normalized to `Interviewer`/`Participant` while names
elsewhere are redacted. 
- For DOCX/TXT → VTT conversions in
